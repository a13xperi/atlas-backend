...
describe("authenticate middleware", () => {
  ...
  it("verifies token against JWT_SECRET env var", () => {
  ...
  it("falls back to dev-secret when JWT_SECRET not set", () => {
    delete process.env.JWT_SECRET;
    const req = makeReq("Bearer valid_token");
    const res = makeRes();
    (mockJwt.verify as jest.Mock).mockReturnValueOnce({ userId: "user-xyz" });
    authenticate(req, res, next as NextFunction);
    expect(mockJwt.verify).toHaveBeenCalledWith("valid_token", "dev-secret");
  });
});
